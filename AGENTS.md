...
starttoscale is a Claude Code plugin for Start to Scale. It is a **distribution artifact** — skills are authored in Workspace Studio and cherry-picked here for client deployment.
...
├── companies/starttoscale/  # Brand data (charter, logos, colors)
...
This plugin defaults to **Start to Scale** brand data at `companies/starttoscale/`. Skills read `charter.json`, `logos/`, `images/`, and `tokens.css` from this path automatically — no prompt-time client selection needed.

For collaborative brands (e.g., "Start to Scale x Partner"), skills fall back to the Start to Scale charter for any missing fields.

**Slug discipline (`client_slug` is `starttoscale`):**
...
- Default brand: `companies/starttoscale/` — hardwired, no discovery needed
