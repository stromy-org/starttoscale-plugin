...
description: "Maintain the starttoscale Claude Code plugin for Start to Scale — wire MCP skill stubs, refresh client-data overlay, add example skills, bump version, push to its marketplace. Use whenever the user wants to extend, modify, or maintain this plugin — even if they don't say 'plugin' explicitly. Triggers on: 'wire skill', 'expose skill', 'add stub', 'refresh client data', 'rebrand the plugin', 'bump plugin version', 'release plugin', 'update marketplace pointer', or any request to change this plugin's behavior."
_local: true
---

# Plugin Maintain — starttoscale

Source of truth for maintaining the **starttoscale** Claude Code plugin (client: Start to Scale). Authored in `claude-plugin-template`; synced into every plugin satellite and into `stromy-org/.claude/skills/plugin-maintain/`.

## When to use

- Wiring a skill stub that points at an MCP-hosted skill.
- Refreshing the client-data overlay (`companies/starttoscale/`).
...
├── companies/
│   └── starttoscale/      # Brand + company data (synced from client-data/)
│       ├── charter.json
│       ├── brand/
│       └── assets/
...
2. Create `skills/<skill>/SKILL.md` as a stub:
   ...
   `companies/starttoscale/`.
   ```
3. Add an entry (or extend an existing one) in `stromy-org/sync-manifest.json` `mcp-skill-mirrors`, appending `starttoscale` to the `plugins` list.
...
Sync pulls from `client-data/clients/starttoscale/` into `companies/starttoscale/` here. Commit via `/conventional-commit`.
...
3. In the matching marketplace repo (`starttoscale-marketplace`), update `marketplace.json` to point at the new plugin commit/tag.
...
- **`companies/starttoscale/` is synced, not authored.** Edits must happen in `client-data/clients/starttoscale/`, then propagate via `sync-client-data.sh`.
...
- Sync pipelines: `stromy-org/scripts/sync-*.sh` (routed by `scripts/sync.sh`)
- Plugin standards: `stromy-org/scaffolds/claude-plugin-template/references/` (agent-surface-contract, tri-agent-parity, three-layer-tooling)
